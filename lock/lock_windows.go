...
func winCreateEphemeral(name string) (windows.Handle, error) {
	...
	handle, err := windows.CreateFile(windows.StringToUTF16Ptr(name), windows.STANDARD_RIGHTS_REQUIRED, 0, nil, windows.OPEN_ALWAYS, FILE_FLAG_DELETE_ON_CLOSE|FILE_FLAG_WRITE_THROUGH, 0)
	if err != nil {
		return 0, err
	}
	return handle, nil
