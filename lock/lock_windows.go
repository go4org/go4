...
func winCreateEphemeral(name string) (windows.Handle, error) {
	...
	handle, err := windows.CreateFile(windows.StringToUTF16Ptr(name), windows.STANDARD_RIGHTS_REQUIRED, 0, nil, windows.OPEN_ALWAYS, FILE_ATTRIBUTE_TEMPORARY|FILE_FLAG_DELETE_ON_CLOSE, 0)
	if err != nil {
		return 0, err
	}
	return handle, nil
