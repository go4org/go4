...
// specification for locating directories.  The specification is at
...
func (d Dir) String() string {
	if d.env == "" {
		panic(errors.New("xdgdir.Dir.String() on zero Dir"))
	}
	return d.env
}
...
func (d Dir) Path() string {
	if d.env == "" {
		panic(errors.New("xdgdir.Dir.Path() on zero Dir"))
	}
	p := d.path()
	if p != "" && d.userOwned {
		info, err := os.Stat(p)
...
func (d Dir) SearchPaths() []string {
	if d.env == "" {
		panic(errors.New("xdgdir.Dir.SearchPaths() on zero Dir"))
	}
	var paths []string
	if p := d.Path(); p != "" {
...
func (d Dir) Open(name string) (*os.File, error) {
	...
	paths := d.SearchPaths()
	if len(paths) == 0 {
		return nil, invalidDirError(fmt.Sprintf("xdgdir: open %s: %s is invalid or not set", name, d.env))
	}
	var firstErr error
	for _, p := range paths {
...
func (d Dir) Create(name string) (*os.File, error) {
	...
	p := d.Path()
	if p == "" {
		return nil, invalidDirError(fmt.Sprintf("xdgdir: create %s: %s is invalid or not set", name, d.env))
	}
...
type invalidDirError string

// IsInvalidDir reports whether e indicates that Open or Create was called on a
// Dir that doesn't have a valid path set in the environment.
func IsInvalidDir(e error) bool {
	_, ok := e.(invalidDirError)
	return ok
}
